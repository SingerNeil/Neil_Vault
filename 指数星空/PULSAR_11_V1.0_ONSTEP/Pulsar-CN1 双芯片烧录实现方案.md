...
*   **物理层**: `PC (上位机)` <==USB==> `CN1 (CP2102)` <==UART0==> `MCU1 (OnStepX)` <==UART2==> `MCU2 (SWS)`
...
这是对现有 OTA 协议的直接应用。PC 直接向 MCU1 发送固件。
...
*   **执行流程**:
    1.  **建立桥梁**: MCU1 收到指令后，进入 **透传模式 (Passthrough Loop)**。
        *   *逻辑*: `while(1) { if(Serial.available) Serial2.write(); if(Serial2.available) Serial.write(); }`
...
1.  **生产效率提升**: 产线烧录或用户升级时，无需拆机，无需连接两个 USB 口，一条线解决所有问题。
