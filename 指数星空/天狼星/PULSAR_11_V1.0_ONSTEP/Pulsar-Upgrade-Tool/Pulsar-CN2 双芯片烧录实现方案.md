...
# Pulsar-CN2 双芯片烧录实现方案与硬件原理分析

**文档版本**: v1.1
**硬件型号**: ESP32-PICO-MINI-02 (MCU1 & MCU2)
**接口标准**: CN2 (Type-C USB-Serial)

---

## 1. 方案概述

本项目旨在实现通过单一物理接口（CN2 Type-C）对 Pulsar 板载的两颗 MCU（MCU1-主控，MCU2-WiFi）进行固件更新。

**核心目标**：
1.  **单口复用**：无需插拔线缆，仅连接 CN2 即可分别烧录两颗芯片。
...
*   **物理层**: 相机（上位机） <== USB-C ==> CN2 <== UART0 ==> MCU1 (OnStepX) <== UART2 ==> MCU2 (SWS)
...
基于上述硬件基础，我们设计了两套通过 CN2 触发的软件协议，分别针对 MCU1 和 MCU2 的烧录。
