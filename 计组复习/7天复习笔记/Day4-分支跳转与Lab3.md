...
- 视频：
  - `5-3-2 指令寻址方式`：可选，只补顺序寻址、跳跃寻址、相对寻址概念。
  - `5-3-1 寻址方式 — 概述`：可选，只看寻址方式分类。
  - `5-3-3 寻址方式 — 操作数寻址方式（1）`、`5-3-3 寻址方式 — 操作数寻址方式（2）`：可选，只补 `lw/sw` 的 base addressing 背景。
  - `5-3-4 寻址方式 — 习题课`：可选，不作为 Lab3 主资料。
  - MIPS `beq/bne/j` 地址计算和 Lab3 填表：直接看 PPT02 p.86-p.96、p.127-p.135、p.139-p.140 和 Lab3。
