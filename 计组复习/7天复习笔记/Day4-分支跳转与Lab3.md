...
- 视频：
  - MIPS `beq/bne/j` 地址计算没有强匹配视频，本日必须以 PPT02 和 Lab3 为主。
  - `5-3-2 指令寻址方式`：只补顺序寻址、跳跃寻址、相对寻址概念。
  - `5-3-1 寻址方式 — 概述`、`5-3-3 寻址方式 — 操作数寻址方式（1）`、`5-3-3 寻址方式 — 操作数寻址方式（2）`、`5-3-4 寻址方式 — 习题课`：只作为寻址方式背景补充，不替代 Lab3。

页码说明：下面的 `PPT p.X` 指 PDF 文件打开后的实际页序。

## 视频匹配度修正

我重新对照了本地视频 p88-p92、PPT02 和 Lab3：

- p88-p92 讲的是通用寻址方式，不是学校 PPT02 里的 MIPS branch/jump 专题。
- Lab3 明确要求 QtSPIM 里的 `bne`、`beq`、`lw`、`sw`、PC、机器指令字段分析；这些要直接看 PPT02 p.86-p.96、p.127-p.135、p.139-p.140。
- `5-3-2 指令寻址方式` 对 “PC-relative addressing” 只有概念帮助，但没有给出 MIPS 的 `PC + 4 + offset * 4` 题型。
- `5-3-3` 对 `lw/sw` 的 base addressing 有背景帮助，但没有覆盖 MIPS I-format 字段和 Lab3 填表。

所以 Day 4 的正确用法是：视频最多先看 `5-3-2` 建概念，真正做题看 PPT02 和 Lab3。
