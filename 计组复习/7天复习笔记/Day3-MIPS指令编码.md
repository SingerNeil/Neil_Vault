...
- 视频：
  - `4-1-5 存储器概述 — 主存中数据的存放`：用来复习 big-endian / little-endian，看完做 Homework2 第 1 题。
  - `5-1 指令系统概述`：可选，只看 ISA 是什么。
  - `5-5 复杂指令集计算机和精简指令集计算机`：可选，只看 RISC / CISC 区别。
  - MIPS R/I/J 编码、机器码转汇编：不要靠视频，直接看 PPT02 p.61、p.75-p.78、p.132-p.135，再做 Homework2 第 2 题。
