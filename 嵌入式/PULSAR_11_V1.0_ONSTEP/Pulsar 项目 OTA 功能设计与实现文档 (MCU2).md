...
1. 用户连接 Pulsar WiFi 热点。
    
2. 浏览器访问 http://192.168.0.1/update.htm。
    
3. 上传 .bin 固件文件。
    
4. 后端流式写入 Flash，完成后自动重启。
...
3. **WiFi 稳定性依赖**：
    ...
    - OTA 过程完全依赖无线信号。如果在写入过程中 WiFi 受到严重干扰断开，虽然不会变砖，但会导致升级失败，需要用户重试。
        
4. **安全性考量**：
    
    - 目前的 /update 接口在局域网内是开放的，没有强制的鉴权机制（如 Token 或密码），连接到 WiFi 的任何人都可以尝试刷写固件。
